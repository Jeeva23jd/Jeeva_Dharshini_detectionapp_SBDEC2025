...
def handle_local_video_upload():
    """Handle local video or image file upload"""
    uploaded_file = st.file_uploader(
        "Choose a video or image file",
        type=['mp4', 'mov', 'avi', 'mkv', 'wmv', 'jpg', 'jpeg', 'png', 'gif', 'bmp', 'tiff'],
        help="Upload a video or image file to analyze for heritage objects"
    )
...
def display_video_controls(video_path, detection_manager):
    ...
    # Check if processing just completed
        ...
        st.rerun()  # Rerun to trigger display_video_results()
...
def display_video_results():
    ...
    # 360-degree rotation visualization
    if 'processed_frames' in st.session_state and st.session_state.processed_frames:
        st.markdown("## 🔄 360° Rotation View")
        st.markdown("View detected objects from all angles using rotated frames:")
        
        # Select a frame to rotate
        frame_options = list(range(min(10, len(st.session_state.processed_frames))))
        ...
        if selected_frame_idx < len(st.session_state.processed_frames):
            base_frame = st.session_state.processed_frames[selected_frame_idx]
            base_frame_rgb = cv2.cvtColor(base_frame, cv2.COLOR_BGR2RGB)
            
            # Create rotation slider
            rotation_angle = st.slider(
                "Rotate the frame (degrees):",
                min_value=0,
                max_value=360,
                value=0,
                step=5
            ...
            rotated_frame = cv2.warpAffine(base_frame_rgb, rotation_matrix, (w, h), borderMode=cv2.BORDER_REFLECT)
            
            # Display rotated frame
            st.image(rotated_frame, caption=f"Frame rotated {rotation_angle}°", use_column_width=True)
            
            # Auto-rotation animation button
            if st.button("▶️ Play 360° Rotation Animation"):
                st.markdown("**Rotating frame through 360 degrees...**")
                rotation_placeholder = st.empty()
                progress_bar = st.progress(0)
                
                for angle in range(0, 361, 6):
                    rotation_matrix = cv2.getRotationMatrix2D(center, angle, 1.0)
                    rotated_frame_anim = cv2.warpAffine(base_frame_rgb, rotation_matrix, (w, h), borderMode=cv2.BORDER_REFLECT)
                    
                    with rotation_placeholder.container():
                        st.image(rotated_frame_anim, caption=f"Rotation: {angle}°", use_column_width=True)
                    
                    progress_bar.progress(angle / 360)
                    time.sleep(0.05)
                
                st.success("✅ 360° rotation complete!")
