...
try:
    ...
    def option_menu(menu_title=None, options=None, icons=None, menu_icon=None, default_index=0, styles=None, **kwargs):
        # Fallback implementation using a simple selectbox when streamlit_option_menu is unavailable
        label = menu_title if menu_title is not None else "Navigation"
        # Use st.selectbox to mimic a menu; return the selected option string
        try:
            return st.selectbox(label, options or [], index=default_index or 0)
        except Exception:
            # If selectbox fails for any reason, return the first option as a safe default
...
try:
    ...
    # reportlab is optional; disable PDF generation when unavailable
...
def generate_pdf_report(stats: Dict, summary_text: str, samples: List = None):
    ...
    if not REPORTLAB_AVAILABLE:
        st.warning("ReportLab is not installed. PDF report generation is disabled. Install with 'pip install reportlab' to enable.")
        return None
