...
if 'detection_manager' not in st.session_state:
    # Create manager instance but do not assume model is loaded yet
    st.session_state.detection_manager = DetectionManager()

detection_manager = st.session_state.detection_manager

# If model not loaded, offer upload UI so user can provide `best.pt`
if detection_manager.model is None:
    st.warning("Model not found.")
    st.info("The app searched these locations for `best.pt` (place the model in any of these or upload below):")
    for p in detection_manager.get_candidate_model_paths():
        st.write(f"- {p}")

    uploaded_model = st.file_uploader("Upload your `best.pt` model file", type=['pt'], help="Upload a trained YOLO .pt model file")
    if uploaded_model is not None:
        with st.spinner("Saving and loading model..."):
            success = detection_manager.save_uploaded_model(uploaded_model)
        if success:
            st.success("Model uploaded and loaded successfully.")
        else:
            st.error("Failed to load the uploaded model. See messages above for details.")
    else:
        st.info("You can also place `best.pt` at the project root to avoid uploading.")
        st.stop()
